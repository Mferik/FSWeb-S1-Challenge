...
function indekseGoreFenomen(fenomenler,index) {
    return `${index}. indekste bulunan fenomen: ${fenomenler[index].profile}`
}
...
function profilListesi(fenomenler) {
  ...
  for (let i=0; i<fenomenler.length; i++){
    profiller.push(fenomenler[i].profile);
  }
return profiller
}
...
function enFenomenler(fenomenler) {
  const enFenomenler= fenomenler.filter(fenomen => fenomen.followers > 100000000);
  return enFenomenler.map(fenomen => fenomen.profile);
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform){
    ...
    for (let i=0; i<fenomenler.length; i++){
    if (fenomenler[i].platform === platform && fenomenler[i].post !=="NA" && fenomenler[i].posts > maxPost){
      
      maxPost = fenomenler[i].posts;
      maxPostName = fenomenler[i].profile;
    }
  }
  return maxPostName
}
